...
> **架构守护**: 本文档专注于用户体验和无障碍设计原则，工程化相关内容请参考 [《工程化与构建指南》](../../reference/engineering/工程化与构建指南.md)。
...
* **库:** 统一使用 `Lucide React`。
    * *版本确立:* 详见 [`package.json`](../package.json) dependencies。
* **规格:** 
    * **Stroke Width:** 统一使用 `Refined` 或 `Standard` 档位，严禁混用。
    * **Size:** 必须引用 `tokens.css` 中的 `--icon-size-*` 变量。
* **实现位置:**
    * 图标组件: `src/popup/components/` (按需导入)
    * 尺寸 Token: `src/popup/styles/tokens.css` (`--icon-size-*`)
    * 具体数值请参考 [`src/design-tokens/tokens.ts`](../src/design-tokens/tokens.ts) 中的 `ICON_SIZES` 对象

### 4.3 响应式策略 (Responsive Strategy)
* **流体布局:** 虽然是 Popup，但也可能面临侧边栏 (Side Panel) 模式。
* **断点:** 
    * **Compact:** 隐藏次要图标，缩小 Padding。
    * **Standard:** 标准布局。
    * **Wide:** 利用横向空间（如两列布局）。
* **实现:**
    * 使用 Tailwind CSS 响应式类 (`sm:`, `md:`, `lg:`)
    * 布局常量定义在 [`src/popup/utils/layoutConstants.ts`](../src/popup/utils/layoutConstants.ts)
    * 设备检测通过 [`src/popup/utils/device.ts`](../src/popup/utils/device.ts) 实现
    * 具体断点数值请参考布局常量文件，严禁在文档中硬编码
