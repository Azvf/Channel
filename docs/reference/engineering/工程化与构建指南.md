...
| **提交代码前** | `npm run check` | TS类型检查 + ESLint + 架构依赖检查 |
...
* **Theme System (Vanilla Extract)**: 自动生成

    * 源: [`src/design-tokens/theme.ts`](../src/design-tokens/theme.ts)

    * 生成逻辑: [`src/design-tokens/theme.css.ts`](../src/design-tokens/theme.css.ts)

    * 产物: 构建时自动生成 CSS 变量（通过 Vite 插件处理）

    * **特性**: Zero-Runtime CSS，所有样式在构建时预计算，运行时只需设置 `data-theme` 属性

    * **架构**: 使用 Vanilla Extract 实现构建时 CSS 生成，详见 [`src/design-tokens/theme.css.ts`](../src/design-tokens/theme.css.ts)
...
- **禁止取巧修复**: 严禁通过延长 `timeout`、增加 `waitFor` 延迟、添加 `setTimeout` 等方式绕过测试失败
- **Root Cause 调试**: 测试失败时必须深入调试，找出根本原因，而非症状修复
- **测试严苛度判定**: 谨慎判定测试用例是否过于严苛。如果是架构设计或功能开发的 bug，必须明确指出并修复，而非降低测试要求

### 7.2 调试流程

当测试失败时，按以下步骤排查：

1. **定位失败原因**: 查看测试输出和错误堆栈，确定是异步逻辑、Mock 配置还是状态更新时机问题
2. **追踪调用链**: 使用 `codebase_search` 和 `grep` 追踪相关代码路径
3. **验证架构合规**: 运行 `npm run check:arch` 检查是否有架构违规导致的问题
4. **分析根本原因**: 区分是代码 bug、架构缺陷还是测试用例不合理

### 7.3 常见问题与修复方法

| 问题类型 | 错误做法 | 正确做法 |
|---------|---------|---------|
| 异步逻辑错误 | 增加 `timeout` 或 `setTimeout` | 修复异步逻辑，确保状态正确更新 |
| Mock 配置不当 | 增加等待时间 | 正确配置 Mock，确保返回预期数据 |
| 状态更新时机 | 手动延迟等待 | 使用 `waitFor`、`findBy*` 等 React Testing Library API |
| 架构设计缺陷 | 降低测试要求 | 修复架构问题，保持测试标准 |

### 7.4 测试严苛度判定

- **不应降低测试要求的情况**:
  - 架构设计缺陷（如依赖循环、层级泄漏）
  - 功能开发 bug（如状态同步失败、数据不一致）
  - 性能问题（如渲染卡顿、内存泄漏）

- **可以调整测试用例的情况**:
  - 测试了实现细节而非行为（如测试内部状态而非用户可见行为）
  - 测试了不应测试的内容（如第三方库的内部实现）
  - 测试用例本身不符合真实用户场景

### 7.5 参考资源

- Jest 配置: [`config/test/jest.config.js`](../../config/test/jest.config.js)
- 测试工具: React Testing Library, Playwright Component Testing
- 调试工具: `codebase_search`, `grep`, `npm run check:arch`
