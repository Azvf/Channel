...
import type { CalendarCell } from '../../shared/types/statsWall';
...
describe('CalendarGridBuilder', () => {
  ...
  it('应该正确处理跨年数据', () => {
    ...
    // 应该包含两个日期
    const dates = layout.cells.map((cell: CalendarCell) => {
  ...
  it('应该正确计算月份标签', () => {
    ...
    // 验证月份标签格式
    layout.months.forEach((month: { label: string; colStart: number }) => {
  ...
  it('应该正确映射活动数据', () => {
    ...
    const cell15 = layout.cells.find((cell: CalendarCell) => {
      const year = cell.date.getFullYear();
      const month = String(cell.date.getMonth() + 1).padStart(2, '0');
      const day = String(cell.date.getDate()).padStart(2, '0');
    ...
    const cell16 = layout.cells.find((cell: CalendarCell) => {
      const year = cell.date.getFullYear();
      const month = String(cell.date.getMonth() + 1).padStart(2, '0');
      const day = String(cell.date.getDate()).padStart(2, '0');
  ...
  describe('CalendarGridBuilder (Boundary Cases)', () => {
    it('闰年处理: 应该正确处理 2月29日', () => {
      ...
      const cell = layout.cells.find((c: CalendarCell) => {
