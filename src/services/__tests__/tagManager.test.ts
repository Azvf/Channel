...
describe('GameplayStore', () => {
  ...
  describe('单例模式', () => {
    it('应该返回相同的实例', () => {
      const instance1 = GameplayStore.getInstance();
      const instance2 = GameplayStore.getInstance();
  ...
  describe('标签创建与管理', () => {
    it('应该创建新标签', () => {
      const tag = store.createTag('前端开发');
    ...
    it('应该创建带自定义颜色的标签', () => {
      ...
      const tag = store.createTag('后端开发', undefined, customColor);
      expect(tag.color).toBe(customColor);
    });

    it('应该创建带描述的标签', () => {
      const description = '前端开发相关技术';
      const tag = store.createTag('前端', description);
    ...
    it('应该验证标签名称', () => {
      ...
      // 正常名称应该成功
      expect(() => store.createTag('前端')).not.toThrow();
    });

    it('应该处理标签名称过长', () => {
      const longName = 'a'.repeat(51);
      expect(() => store.createTag(longName)).toThrow();
    });

    it('应该自动生成标签ID', () => {
      const tag = store.createTag('测试标签名称');
    ...
    it('应该根据ID获取标签', () => {
      const createdTag = store.createTag('前端');
      const foundTag = store.getTagById(createdTag.id);
    ...
    it('应该根据名称查找标签（忽略大小写）', () => {
      store.createTag('前端开发');
      
      expect(store.findTagByName('前端开发')).toBeDefined();
      expect(store.findTagByName('前端开发')).toBeDefined();
      expect(store.findTagByName('前端开发')).toBeDefined();
      expect(store.findTagByName('后端开发')).toBeUndefined();
    });

    it('应该处理不存在的标签', () => {
      expect(store.getTagById('不存在的标签')).toBeUndefined();
      expect(store.findTagByName('不存在的标签')).toBeUndefined();
    });
  });
  ...
  describe('标签绑定功能', () => {
    ...
    beforeEach(() => {
      tagA = store.createTag('前端');
      tagB = store.createTag('React');
      tagC = store.createTag('Vue');
    });

    it('应该成功绑定两个标签', () => {
      const result = store.bindTags(tagA.id, tagB.id);
    ...
    it('应该实现对称绑定', () => {
      store.bindTags(tagA.id, tagB.id);
      
      const boundTagsA = store.getBoundTags(tagA.id);
      const boundTagsB = store.getBoundTags(tagB.id);
    ...
    it('应该防止标签绑定自身', () => {
      const result = store.bindTags(tagA.id, tagA.id);
    ...
    it('应该解除标签绑定', () => {
      ...
      const result = store.unbindTags(tagA.id, tagB.id);
    ...
    it('应该处理解除不存在标签的绑定', () => {
      const result = store.unbindTags(tagA.id, 'nonexistent-id');
      expect(result).toBe(false);
    });

    it('应该支持多个标签绑定', () => {
      store.bindTags(tagA.id, tagB.id);
      store.bindTags(tagA.id, tagC.id);
      
      const boundTags = store.getBoundTags(tagA.id);
  ...
  describe('页面管理', () => {
    ...
    it('应该更新已存在的页面', () => {
      ...
      const page1 = store.createOrUpdatePage(url, 'GitHub', 'github.com');
      const page2 = store.createOrUpdatePage(url, 'GitHub Repository', 'github.com');
    ...
    it('应该根据ID获取页面', () => {
      const createdPage = store.createOrUpdatePage(
        'https://github.com',
        'GitHub',
        'github.com'
      );
      
      const foundPage = store.getPageById(createdPage.id);
      expect(foundPage).toBeDefined();
      expect(foundPage?.title).toBe('GitHub');
    });

    it('应该返回所有带标签的页面', () => {
      const page1 = store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
      store.createOrUpdatePage('https://google.com', 'Google', 'google.com');
      
      // 只为第一个页面添加标签
      const tag = store.createTag('开源');
      store.addTagToPage(page1.id, tag.id);
      
      const taggedPages = store.getTaggedPages();
  ...
  describe('页面标签关联', () => {
    let page: ReturnType<typeof store.createOrUpdatePage>;
    let tag: GameplayTag;

    beforeEach(() => {
      page = store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
      tag = store.createTag('开源');
    });

    it('应该添加标签到页面', () => {
      const result = store.addTagToPage(page.id, tag.id);
    ...
    it('应该从页面移除标签', () => {
      ...
      const result = store.removeTagFromPage(page.id, tag.id);
    ...
    it('应该支持toggle接口', () => {
      ...
      store.toggleTagOnPage(page.id, tag.id, true);
      expect(page.tags).toContain(tag.id);
      
      store.toggleTagOnPage(page.id, tag.id, false);
      expect(page.tags).not.toContain(tag.id);
    });

    it('应该根据标签筛选页面', () => {
      const page2 = store.createOrUpdatePage('https://google.com', 'Google', 'google.com');
      const page3 = store.createOrUpdatePage('https://stackoverflow.com', 'Stack Overflow', 'stackoverflow.com');
      
      // 为前两个页面添加标签
      store.addTagToPage(page.id, tag.id);
      store.addTagToPage(page2.id, tag.id);
      
      const taggedPages = store.getTaggedPages(tag.id);
    ...
    it('应该返回多个标签的页面（同时满足多个标签）', () => {
      const tag2 = store.createTag('JavaScript');
      ...
      const pagesWithTag = store.getTaggedPages(tag.id);
  ...
  describe('高级功能', () => {
    it('应该创建标签并添加到页面', () => {
      const page = store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
      
      const tag = store.createTagAndAddToPage('开源', page.id);
    ...
    it('应该处理同名标签（使用现有标签）', () => {
      ...
      const tag1 = store.createTagAndAddToPage('开源', page1.id);
      const tag2 = store.createTagAndAddToPage('开源', page2.id);
    ...
    it('应该提供数据统计', () => {
      store.createTag('前端');
      store.createTag('后端');
      store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
    ...
    it('应该清空所有数据', () => {
      ...
      expect(store.getDataStats().pagesCount).toBeGreaterThan(0);
      
      store.clearAllData();
      
      expect(store.getDataStats().tagsCount).toBe(0);
      expect(store.getDataStats().pagesCount).toBe(0);
    });
  });
  ...
  describe('边界情况', () => {
    ...
    it('应该处理无效的页面ID', () => {
      const tag = store.createTag('前端');
      const result = store.addTagToPage('invalid-page-id', tag.id);
      expect(result).toBe(false);
    });

    it('应该处理无效的标签ID', () => {
      const page = store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
      const result = store.addTagToPage(page.id, 'invalid-tag-id');
      expect(result).toBe(false);
    });

    it('应该处理绑定不存在的标签', () => {
      const tag = store.createTag('前端');
      const result = store.bindTags(tag.id, 'nonexistent-id');
      expect(result).toBe(false);
    });

    it('应该处理中文和特殊字符', () => {
      const chineseTag = store.createTag('中文标签');
      const specialCharTag = store.createTag('Tag_With-Special.Characters');
  ...
  describe('清理功能', () => {
    it('应该清理未使用的标签', () => {
      ...
      expect(store.getAllTags().length).toBe(3);
      
      store.cleanupUnusedTags();
      
      // 应该只保留tag1
      const remainingTags = store.getAllTags();
    ...
    it('应该不清理正在使用的标签', () => {
      ...
      expect(store.getTagById(tag.id)).toBeDefined();
    });
  });
  ...
  describe('导入导出功能', () => {
    beforeEach(() => {
      // 初始化一些测试数据
      const tag1 = store.createTag('前端', '前端开发相关', '#FF5733');
      const tag2 = store.createTag('后端', '后端开发相关', '#33FF57');
      store.bindTags(tag1.id, tag2.id);
      
      const page1 = store.createOrUpdatePage(
        'https://github.com',
        'GitHub',
        'github.com',
        'https://github.com/favicon.ico'
      );
      const page2 = store.createOrUpdatePage(
    ...
    it('应该导入并覆盖现有数据', async () => {
      ...
      // 导入数据（覆盖模式）
      const result = await store.importData(exportData, false);
    ...
    it('应该导入并合并到现有数据', async () => {
      const originalExportData = store.exportData();
      
      // 清空并重新创建一些数据
      store.clearAllData();
      const newTag = store.createTag('数据库');
      const newPage = store.createOrUpdatePage(
        'https://mongodb.com',
        'MongoDB',
        'mongodb.com'
      );
      store.addTagToPage(newPage.id, newTag.id);
      
      // 导入原始数据（合并模式）
      const result = await store.importData(originalExportData, true);
      
      expect(result.success).toBe(true);
      
      // 应该有原来的2个标签 + 新添加的1个标签 = 3个
      expect(store.getDataStats().tagsCount).toBe(3);
      // 应该有原来的2个页面 + 新添加的1个页面 = 3个
      expect(store.getDataStats().pagesCount).toBe(3);
      
      // 验证新添加的数据还在
      expect(store.findTagByName('数据库')).toBeDefined();
    });

    it('合并模式应该不覆盖现有数据', async () => {
      // 创建一些初始数据
      store.clearAllData();
      const tag1 = store.createTag('前端', '原始描述', '#FF0000');
      const page = store.createOrUpdatePage(
        'https://github.com',
        'GitHub Original',
        'github.com'
      ...
      const initialStats = store.getDataStats();
      ...
      // 导入（合并模式）
      await store.importData(importData, true);
      
      // 应该保留原始的标签（不覆盖）
      const preservedTag = store.getTagById(tag1.id);
      expect(preservedTag?.description).toBe('原始描述');
      expect(preservedTag?.color).toBe('#FF0000');
      
      // 应该添加新的标签
      expect(store.findTagByName('后端')).toBeDefined();
      
      // 最终应该有2个标签（1个原始 + 1个新添加）
      expect(store.getDataStats().tagsCount).toBe(2);
    });

    it('应该拒绝导入无效的 JSON 格式', async () => {
      const invalidJson = '这不是有效的JSON{';
      const result = await store.importData(invalidJson, false);
    ...
    it('应该拒绝导入缺少必需字段的数据', async () => {
      ...
      const result = await store.importData(invalidData, false);
    ...
    it('合并模式在 ID 冲突时应保留现有数据并合并新数据', async () => {
      store.clearAllData();
      const existingTag = store.createTag('Existing Tag', '原始描述', '#123456');
      const existingPage = store.createOrUpdatePage(
        'https://existing.example.com',
        'Existing Page',
        'existing.example.com'
      );
      store.addTagToPage(existingPage.id, existingTag.id);
      ...
      const result = await store.importData(importPayload, true);

      expect(result.success).toBe(true);

      const preservedTag = store.getTagById(existingTag.id);
      ...
      const preservedPage = store.getPageById(existingPage.id);
      ...
      expect(store.getTagById('tag_new')).toBeDefined();
      expect(store.getPageById('page_new')).toBeDefined();
    });
    ...
    it('导入缺失引用的数据集后应保持可用性', async () => {
      ...
      const result = await store.importData(payloadWithDanglingRefs, false);

      expect(result.success).toBe(true);
      expect(() => store.getAllTagUsageCounts()).not.toThrow();
      expect(() => store.getTaggedPages('missing_tag')).not.toThrow();

      const usageCounts = store.getAllTagUsageCounts();
      expect(usageCounts.missing_tag).toBeUndefined();

      const danglingPage = store.getPageById('page_dangling');
      expect(danglingPage).toBeDefined();
      expect(danglingPage?.tags).toContain('missing_tag');

      expect(store.deleteTag('missing_tag')).toBe(false);
      expect(() => store.createTag('恢复正常')).not.toThrow();
    });

    it('完整的导出导入流程应该保持数据一致性', async () => {
      const originalStats = store.getDataStats();
      ...
      // 清空并重新导入
      store.clearAllData();
      const importResult = await store.importData(exportData, false);
      
      expect(importResult.success).toBe(true);
      
      // 验证数据统计一致
      const newStats = store.getDataStats();
      ...
      for (const tagId in parsed.tags) {
        const importedTag = store.getTagById(tagId);
  ...
  describe('标签验证功能', () => {
    it('应该验证空字符串标签名', () => {
      const result = store.validateTagName('');
    ...
    it('应该验证超过50字符的标签名', () => {
      const longName = 'a'.repeat(51);
      const result = store.validateTagName(longName);
      expect(result.valid).toBe(false);
      expect(result.error).toBe('标签名称不能超过50个字符');
    });

    it('应该验证有效的标签名', () => {
      const result = store.validateTagName('前端开发');
  ...
  describe('createTagAndAddToPage 错误处理', () => {
    it('应该处理无效标签名', () => {
      const page = store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
      expect(() => store.createTagAndAddToPage('', page.id)).toThrow();
    });
  });
  ...
  describe('页面标题更新', () => {
    it('应该更新页面标题', () => {
      const page = store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
      const result = store.updatePageTitle(page.id, 'GitHub - Home');
      expect(result).toBe(true);
    ...
    it('应该处理不存在的页面ID', () => {
      const result = store.updatePageTitle('nonexistent-id', 'New Title');
      expect(result).toBe(false);
    });

    it('应该处理空标题', () => {
      const page = store.createOrUpdatePage('https://github.com', 'GitHub', 'github.com');
      const result = store.updatePageTitle(page.id, '   ');
      expect(result).toBe(false);
    });
  ...
  describe('deleteTag 绑定关系清理', () => {
    it('deleteTag 应该级联清理所有引用并保持其他数据完整', () => {
      const tagA = store.createTag('TagA');
      const tagB = store.createTag('TagB');
      const pageA = store.createOrUpdatePage('https://a.example.com', 'Page A', 'a.example.com');
      const pageB = store.createOrUpdatePage('https://b.example.com', 'Page B', 'b.example.com');

      store.bindTags(tagA.id, tagB.id);

      store.addTagToPage(pageA.id, tagA.id);
      store.addTagToPage(pageB.id, tagA.id);

      const deleteResult = store.deleteTag(tagA.id);

      expect(deleteResult).toBe(true);
      expect(store.getTagById(tagA.id)).toBeUndefined();

      const remainingTagB = store.getTagById(tagB.id);
      expect(remainingTagB).toBeDefined();
      expect(remainingTagB?.bindings).not.toContain(tagA.id);

      const updatedPageA = store.getPageById(pageA.id);
      const updatedPageB = store.getPageById(pageB.id);
    ...
    it('应该清理标签的绑定关系', () => {
      ...
      const tag3 = store.createTag('数据库');
      ...
      store.addTagToPage(page1.id, tag1.id);
      store.addTagToPage(page2.id, tag2.id);
      store.addTagToPage(page3.id, tag3.id);
      
      // 移除 tag1 的页面关联，然后清理未使用的标签
      store.removeTagFromPage(page1.id, tag1.id);
      store.cleanupUnusedTags();
      
      // tag1 应该被删除，tag2 和 tag3 应该保留
      expect(store.getTagById(tag1.id)).toBeUndefined();
      expect(store.getTagById(tag2.id)).toBeDefined();
      expect(store.getTagById(tag3.id)).toBeDefined();
      
      // 检查绑定关系是否被清理
      const tag2After = store.getTagById(tag2.id);
      const tag3After = store.getTagById(tag3.id);
    ...
    it('应该处理不存在的标签', () => {
      ...
      expect(() => store.cleanupUnusedTags()).not.toThrow();
    });
  });
